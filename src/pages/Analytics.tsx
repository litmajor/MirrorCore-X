...
import { TrendingUp, DollarSign, Target, AlertTriangle, BarChart3 } from 'lucide-react';
import { useAPI } from '../hooks/useAPI';
import { MetricCardSkeleton, ChartSkeleton } from '../components/Skeleton';

const Analytics: React.FC = () => {
  const { data: performance, loading: perfLoading, error: perfError } = useAPI('/api/performance/summary');
  const { data: strategies, loading: stratLoading } = useAPI('/api/strategies');
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {perfLoading ? (
          ...
          <div className="col-span-full brand-card text-center py-12">
            <BarChart3 className="w-16 h-16 text-txt-secondary mx-auto mb-4" />
            <h3 className="text-xl font-semibold text-white mb-2">Unable to Load Analytics</h3>
            <p className="text-txt-secondary">Start the backend server to view performance metrics.</p>
          </div>
        ) : (
          <>
